...
class Machine:
    ...
    def nb_connect(cls, start: str, end: str, must_visit: list[str] = []):
        ...
        must_visit_ids = [Machine.machines[machine_name].id_machine for machine_name in must_visit]
                    ...
                    if id_output_machine == id_end_machine:
                        for must_visit_id in must_visit_ids:
                            if must_visit_id not in list_id_machine:
                                continue
                            print([Machine.machines_from_id[id_machine].name for id_machine in list_id_machine])
                        nb_path += 1
                        continue

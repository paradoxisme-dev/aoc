...
class Machine:
    ...
    def nb_connect(cls, start: str, end: str, must_visit: list[str] = []):
        ...
        paths: dict[int, list[int]] = {
            machine.id_machine : [(1, set()), [machine.id_machine]] for machine in start_machine.output
        }
        iter_nb = 0
        while len(paths) > 0:
            iter_nb += 1
            new_paths: dict[int, list[int]] = {}
            for (nb_path_to, must_visited), list_id_machine in paths.items():
                list_id_machine = list_id_machine[1]
                current_machine = Machine.machines_from_id[list_id_machine[-1]]
                if current_machine.id_machine in must_visit_ids:
                    must_visited.add(current_machine.id_machine)
                for output_machine in current_machine.output:
                    ...
                    if id_output_machine == id_end_machine:
                        if len(must_visit) == len(must_visited):
                            nb_path += nb_path_to
                        continue
                    if id_output_machine in list_id_machine:
                        continue
                    if (current_machine.id_machine, must_visited) not in new_paths:
                        new_path = list(list_id_machine)
                        new_path.append(id_output_machine)
                        new_paths[(current_machine.id_machine, must_visited)] = [1, new_path]
                    new_paths[(current_machine.id_machine, must_visited)][0] *= nb_path_to
...
machine_connexion = {}
with open('input_ex2.txt', 'r') as file:
    for line in file:
        line = line[:-1]
...
print(Machine.nb_connect('dac', 'out', ['dac', 'fft']))
